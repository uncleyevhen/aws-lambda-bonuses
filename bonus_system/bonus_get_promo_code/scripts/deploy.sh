...
FUNCTION_NAME="bonus-promo-get-prod"
LAMBDA_ROLE_NAME="lambda-promo-role"
HANDLER="lambda_function.lambda_handler"
RUNTIME="python3.13"
ZIP_FILE="bonus-promo-get-prod-deployment-package.zip"
BUILD_DIR="build"
...
if ! aws lambda get-function --function-name "$FUNCTION_NAME" --region "$AWS_REGION" >/dev/null 2>&1; then
    ...
    aws lambda create-function \
        ...
        --timeout 60 \
        --memory-size 256 \
        --environment "Variables={REPLENISH_FUNCTION_NAME=bonus-replenish-promo-prod,SESSION_S3_BUCKET=lambda-promo-sessions,BATCH_THRESHOLD=20,MIN_CODES_THRESHOLD=3}" >/dev/null
