(function(){
  ...
  function initializeCartObserver() {
    ...
    // Оптимізований спостерігач - спостерігаємо лише за document.body
    var observer = new MutationObserver(function(mutations) {
      ...
      // Оптимізована обробка мутацій - перевіряємо лише необхідні умови
      for (var i = 0; i < mutations.length; i++) {
        ...
        if (mutation.type === 'childList') {
          ...
          for (var j = 0; j < mutation.removedNodes.length; j++) {
            ...
            if (node.nodeType === 1) {
              if ((node.id === 'bonus-block' || (node.querySelector && node.querySelector('#bonus-block'))) && 
                ...
                // Якщо знайшли що шукали, виходимо з циклу
                break;
              }
            }
          }
          
          // Якщо ще не знайшли потрібне, перевіряємо додані вузли
          if (!shouldCheckBonusBlock || !shouldReplaceText || !shouldCheckElementsState || !shouldSetupPromoListener) {
            for (var k = 0; k < mutation.addedNodes.length; k++) {
              ...
              if (node.nodeType === 1) {
                // Перевірка для заміни тексту (оптимізована)
                ...
                // Перевірка нових кнопок видалення товарів
                if ((node.matches && (node.matches('.order-i-remove') || node.matches('.j-remove-p'))) || 
                  ...
                  // Якщо застосовані бонуси, блокуємо нові кнопки
                  if (isBonusPromoCodeApplied()) {
                    setTimeout(function() {
                      disableProductRemoval();
                    }, 100);
                ...
                // Перевірка для блоку бонусів
                if (!shouldCheckBonusBlock && (node.id === 'bonus-block' || (node.matches && (node.matches('.order-details') || node.matches('.order-details__block') || node.matches('.order-details-i'))))) {
                  shouldCheckBonusBlock = true;
                }
                ...
                // Якщо знайшли все, що потрібно, виходимо з циклу
                if (shouldReplaceText && shouldCheckElementsState && shouldCheckBonusBlock && shouldSetupPromoListener) {
                  break;
                }
              }
            }
        ...
        // Відстежуємо зміни атрибутів елементів бонусів (оптимізована перевірка)
        else if (!shouldCheckElementsState && mutation.type === 'attributes' && mutation.target && mutation.target.closest && 
      ...
      if (shouldReplaceText) {
        var now = Date.now();
        // Обмежуємо частоту заміни тексту
        if (now - lastTextReplace > 100) {
          lastTextReplace = now;
          setTimeout(function() {
            log('Заміна тексту через MutationObserver');
            instantTextReplace();
            ensurePhoneLockState();
          }, 10);
        }
      ...
      if (shouldCheckElementsState) {
        var now = Date.now();
        // Обмежуємо частоту перевірки стану елементів
      ...
      if (shouldCheckBonusBlock && window.location.href.includes('/checkout/')) {
        var now = Date.now();
        // Обмежуємо частоту перевірки блоку бонусів
    ...
    // Оптимізоване спостереження - спостерігаємо лише за document.body з обмеженими параметрами
    observer.observe(document.body, { 
      childList: true, 
      subtree: true,
      attributes: false // Вимикаємо спостереження за атрибутами на загальному рівні
    });
  }

  function initBonusSystem() {
    log('Ініціалізація системи бонусів');
    ...
    log('Ініціалізація спостерігача за змінами');
    initializeCartObserver();
    
    // Видаляємо періодичну перевірку, замінюючи її на події
    // Перевірки будуть виконуватись лише за потреби через слухачі подій
